...
namespace SignalRWebUI.Controllers
{
	public class AboutController : Controller
	{
		...
		public async Task<IActionResult> CreateAbout(CreateAboutDtos createAboutDtos)
		{
			...
			var responseMessage = await client.PutAsync("https://localhost:7035/api/About", stringContent);
			if(responseMessage.IsSuccessStatusCode)
			{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7035/api/About/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"https://localhost:7035/api/About/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
