...
namespace SignalRWebUI.Controllers
{
	public class FeatureController : Controller
	{
			...
			public async Task<IActionResult> CreateFeature(CreateFeatureDtos createFeatureDtos)
			{
				...
				var responseMessage = await client.PutAsync("https://localhost:7035/api/Feature", stringContent);
				if (responseMessage.IsSuccessStatusCode)
				{
			...
			{
				var client = _httpClientFactory.CreateClient();
				var responseMessage = await client.DeleteAsync($"https://localhost:7035/api/Feature/{id}");
				if (responseMessage.IsSuccessStatusCode)
				{
					return RedirectToAction("Index");
				}
			...
			{
				var client = _httpClientFactory.CreateClient();
				var responseMessage = await client.GetAsync($"https://localhost:7035/api/Feature/{id}");
				if (responseMessage.IsSuccessStatusCode)
				{
					var jsonData = await responseMessage.Content.ReadAsStringAsync();
